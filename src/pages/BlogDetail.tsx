...
export default function Article() {
    ...
    fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(`https://medium.com/feed/@jayanthansenthilkumar?t=${Date.now()}`)}`)
      ...
      .then(data => {
        if (data.items) {
          ...
          if (foundArticle) {
            document.title = `${foundArticle.title} | Portfolio`;
          }
        }
        setIsLoading(false);
      })
  ...
  if (!article) {
    return (
      <div className="py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 bg-[#F6F4EB] min-h-screen text-center">
  ...
  return (
    <div className="py-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 bg-[#F6F4EB] min-h-screen">
      <motion.div
